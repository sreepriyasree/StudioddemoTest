...
public class CreateSchemaTC extends BaseClass {
    ...
    public void createschema() throws InterruptedException, IOException {
        ...
        if (createSpaceBtn != null) {
            ...
            // Generate timestamped schema title
            ...
            WebElement nextBtn = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("button[class*='next-btn']")));

            // Debug: Print the class of the button
            System.out.println("Next button class before clicking: " + nextBtn.getAttribute("class"));

            // Wait until 'disabled' class is removed
            wait.until(ExpectedConditions.not(ExpectedConditions.attributeContains(nextBtn, "class", "disabled")));

            // Click 'Next' button (with JavaScript fallback)
            try {
                nextBtn.click();
            } catch (Exception e) {
            ...
            WebElement SaveSchemaBtn = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("div[class='position-next-save'] button[type='submit']")));
            SaveSchemaBtn.click();
            Thread.sleep(20000);
